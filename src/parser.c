...
typedef struct size_params{
    ...
    int c;
    int index;
    int time_steps;//以上参数都network中的参数。
    network net;
} size_params;
...
{//返回配置文件中的所有配置，信息。链表sections可以把所有cfg中的信息都串起来。
    ...
    section *s = (section *)n->val;//->优先级高于类型转换，因此，这里是将node* n的void* val部分，强制转化成section。
...
*/
    ...
    n = n->next;//开始指向cfg中的第二部分，即开始Layer的解析。n仍然是指向Node类型的指针。
    int count = 0;
    free_section(s);//释放掉第一份申请的空间。
    ...
    while(n){
        ...
        }//确定该section的类型，也就是层的类型，并完成相应层的参数配置。
...
list *read_cfg(char *filename)
{
    ...
    char *line;
    int nu = 0;
    list *sections = make_list();//双向链表
    section *current = 0;
    while((line=fgetl(file)) != 0){//fgetl获取某一行，中间添加了复杂的保护机制，防止有字符串没有读进来。
        ++ nu;
        strip(line);//去除各种空格
        switch(line[0]){
